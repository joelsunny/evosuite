...
basefolder="/home/sunny/prantiks-evosuite-and-d4j/exps"
...
#		mkdir -p /home/abhijitc/neural_project/results/evo_itr_dump/Closure/temp2/bug"$i"/"$j"
		...
		mkdir -p "$basefolder/$bugfolder/$iteration"/fl
			...
			"$d4j_home"/defects4j/framework/bin/run_fault_localization.pl -p Chart -d "$basefolder/$bugfolder/$iteration"/Chart/evosuite-VCMDDU2/"$bugid" \
			-o "$basefolder/$bugfolder/$iteration"/fl -t "$basefolder/$fl_temp/$iteration" \
			-i  "$d4j_home"/defects4j/framework/projects/Chart/modified_classes/"$bugid".src -y sfl -e ochiai -g line
